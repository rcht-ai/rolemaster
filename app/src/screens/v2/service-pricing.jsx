...
export function ScreenV2ServicePricing({ lang, setLang, onLogout }) {
  ...
  const requiredChecks = [
    [vals.service.demo_mode,            lang === 'zh' ? '演示偏好' : 'Demo preferences',           (v) => Array.isArray(v) && v.length > 0],
    [vals.service.sales_assist_level,   lang === 'zh' ? '销售配合意愿' : 'Sales-assist',           (v) => v && String(v).trim().length > 0],
    [vals.service.sales_coverage_regions, lang === 'zh' ? '销售辐射区域' : 'Sales coverage',       (v) => Array.isArray(v) && v.length > 0],
    [vals.service.delivery_scope,       lang === 'zh' ? '交付范围' : 'Delivery scope',             (v) => Array.isArray(v) && v.length > 0],
    [vals.service.support_languages,    lang === 'zh' ? '客户服务语言' : 'Support languages',      (v) => Array.isArray(v) && v.length > 0],
    [vals.pricing.pricing_model,        lang === 'zh' ? '定价模式' : 'Pricing model',              (v) => v && String(v).trim().length > 0],
    [vals.pricing.cost_price,           lang === 'zh' ? '成本价' : 'Cost price',                   (v) => v && String(v).trim().length > 0],
    [vals.pricing.suggested_retail,     lang === 'zh' ? '建议零售价' : 'Suggested retail',         (v) => v && String(v).trim().length > 0],
  ];

  const submitAll = async () => {
    const missing = requiredChecks.filter(([v, , chk]) => !chk(v)).map(([, label]) => label);
    if (missing.length > 0) {
      setErr(lang === 'zh'
        ? `还有 ${missing.length} 项必填未填:${missing.join('、')}`
  ...
  return (
    <div className="screen-anim platform-supplier v2" style={{ minHeight: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div className="v2-page">
        <div className="v2-eyebrow">{lang === 'zh' ? '步骤 6 · 共享' : 'Step 6 · Shared'}</div>
        <div className="v2-title-row">
          <h1 className="v2-display">{lang === 'zh' ? '服务与价格' : 'Service & pricing'}</h1>
          <span className="v2-status-pill v2-status-pill--ai">🔗 {lang === 'zh' ? '所有岗位共享' : 'Shared across Roles'}</span>
        </div>
        <p className="v2-lede">
        ...
        <div className="v2-input-card" style={{ marginBottom: 14 }}>
          <div className="v2-h3">{lang === 'zh' ? '服务' : 'Service'}</div>
        ...
        {/* Pricing */}
        <div className="v2-input-card" style={{ marginBottom: 14 }}>
          <div className="v2-h3">{lang === 'zh' ? '价格' : 'Pricing'}</div>
                  ...
                  <div key={top.id} style={{ marginBottom: 4 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 6px' }}>
                      ...
                      <label style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer', fontSize: 12 }}>
                        <input type="checkbox" checked={selected.includes(top.id)} onChange={() => toggle(top.id)} style={{ width: 16, height: 16, accentColor: 'var(--plat-supplier)', cursor: 'pointer' }} />
                      ...
                      return (
                        <div key={child.id} style={{ marginLeft: 18 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '3px 6px' }}>
                            ...
                            <label style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer', fontSize: 12 }}>
                              <input type="checkbox" checked={selected.includes(child.id)} onChange={() => toggle(child.id)} style={{ width: 16, height: 16, accentColor: 'var(--plat-supplier)', cursor: 'pointer' }} />
                              <span>{lbl(child)}</span>
                            </label>
                          </div>
                          {isExp2 && grand.map(g => (
                            <div key={g.id} style={{ marginLeft: 22, padding: '3px 6px' }}>
                              <label style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer', fontSize: 11.5 }}>
                                <input type="checkbox" checked={selected.includes(g.id)} onChange={() => toggle(g.id)} style={{ width: 16, height: 16, accentColor: 'var(--plat-supplier)', cursor: 'pointer' }} />
                                <span>{lbl(g)}</span>
                              </label>
                            </div>
                          ))}
...
// Mini visible checkbox box for the search-results list inside RegionPicker.
function RegionCheckbox({ checked }) {
  return (
    <span style={{
      width: 16, height: 16, flexShrink: 0,
      borderRadius: 4,
      border: checked ? '1.5px solid var(--plat-supplier)' : '1.5px solid var(--line)',
      background: checked ? 'var(--plat-supplier)' : 'white',

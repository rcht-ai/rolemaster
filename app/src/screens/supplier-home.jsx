...
export function ScreenSupplierHome({ lang, setLang, onLogout }) {
  ...
  return (
    <div className="platform-supplier v2 hero-bg" style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
          ...
          {items.map(it => {
            ...
            return (
              <div key={it.id} style={{
                ...
                {/* Action button (Continue/View) — fixed-width column so all rows align vertically */}
                <div style={{ width: 96, flexShrink: 0, textAlign: 'right' }}>
                  <button onClick={() => onOpen(it)} className="v2-btn-link"
                    style={{ padding: 0, fontSize: 13 }}>
                    {it.status === 'submitted' ? (lang === 'zh' ? '查看' : 'View') : (lang === 'zh' ? '继续' : 'Continue')} →
                  </button>
                </div>
                {/* Delete — fixed-width column so it's always present (just empty when not deletable) */}
